...
const valuesToRows = (values: any, initialFields: any) => {
...
const DynamicFields = ({
  ...
  const [rows, setRows] = useState(valuesToRows(values, initialFields));
  const [inputIndex, setInputIndex] = useState(0);
  const [errors, setErrors] = useState<any>();

  const device: string = useContext(DeviceContext);
  //////////console.log("ROWS", rows)
  ////////////////////console.log("DynamicFields Rendered", rows)
  ...
  const handleFieldChange = (
    ...
    ////////////////////////////////////////////////////////////////////console.log("FIELDNAME", fieldName)
    const isValid = validateDynamicFieldsProperty(
      value,
      index,
      fieldName,
      component,
    ...
    if (submitIndex > 0) {
      ...
      values.map((row: any, index: number) => {
        ...
        Object.keys(row).map((key2: string) => {
          //const { required, realNumber } = schema.fields[key2].validators;
          if (
            // (required == true || required?.value == true) &&
            (row[key2] == "" || isObjectEmpty(row[key2]) == true || row[key2]._id == "")
          ) {
            tempErrors = {
              ...tempErrors,
              [key2 + "_" + index]: getDefaultValidationMessage("required", key2, "", "" )
            };
          } else if (
            // (realNumber == true || realNumber?.value == true) &&
            isRealNumber(row[key2]) == false
          ) {
            //////console.log("the key2", key2);
            tempErrors = {
              ...tempErrors,
              [key2 + "_" + index]: getDefaultValidationMessage("realNumber", "", "", "" ),
            };
          } else {
            if (row[key2]) {
  ...
  const validateDynamicFieldsProperty = (
    value: any,
    index: number,
    fieldName: string,
    component: string,
    ...
    if (validators) {
      ...
      if (
        ...
        setErrors((prev: any) => {
          return {
            ...prev,
            [targetField]: getDefaultValidationMessage("required", fieldName, "", "" )
          };
        });
      ...
      if (
        ...
        setErrors((prev: any) => {
          return {
            ...prev,
            [targetField]:
              getDefaultValidationMessage("realNumber", "", "", "" ),
          };
        });
          ...
          rows.map((row: any, index: number) => {
            ...
            return (
              <SRow>
                ...
                <SRelativeDiv $size={size}>
                  <SAbsoluteDiv $device={device}>
                    {index <= maxRows - 2 && index == rows.length - 1 && (
                      <IconButton
